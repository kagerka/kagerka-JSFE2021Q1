  ...
  clickCounter(): unknown {
    ...
    cardField?.addEventListener('click', (event) => {
      if (event.target instanceof HTMLElement) {
        click++;
        if (
          event.target.classList.contains('card__back')
          && click % 2 === 1
          ...
          firstClick?.previousElementSibling?.classList.add(
            'clicked',
            'firstClick',
          );
          firstClick = document.querySelector(
            '.card__front.clicked.firstClick',
          );
          ...
          secondClick = document.querySelector(
            '.card__back.clicked.secondClick',
          );
          secondClick?.previousElementSibling?.classList.add(
            'clicked',
            'secondClick',
          );
          secondClick = document.querySelector(
            '.card__front.clicked.secondClick',
          );
        ...
        if (click === 0) {
          if (
            firstClick?.style.backgroundImage
            === secondClick?.style.backgroundImage
          ) {
            clickTotal++;
            setTimeout(() => {
      ...
      const clickDiffField = document.getElementById(
        'clickDiffField',
      ) as HTMLElement | null;
